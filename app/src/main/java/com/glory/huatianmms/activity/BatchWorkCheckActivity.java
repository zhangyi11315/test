...
public class BatchWorkCheckActivity extends FreeUI_BaseActivity {
    ...
    protected void initView() {
        initDatePicker();
        rvThingList.setLayoutManager(new LinearLayoutManager(BatchWorkCheckActivity.this));
        rvThingList.setItemAnimator(new DefaultItemAnimator());
        rvThingList.addItemDecoration(new DividerItemDecoration(BatchWorkCheckActivity.this, 1));
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (start == 0 && before == 0 && count > 1) {
                    if (s.length() > 0) {
                        ...
                        if (!isHave) {
                            etLot.setText("");
                            showToast("批次号不存在于列表中,请重新扫描", false);
                        }
                    } else {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvSearch:
                    ...
                    public void onSuccess
                        ...
                        if (response.body().isSuccess(getApplicationContext())) {
                            ...
                            if (list.size() == 0) {
                                ...
                                billPop = FreeApi_PopUtils.getNormalPop(BatchWorkCheckActivity.this,
                ...
                sb.append("docType = 'STOCKTAKING' AND docStatus = 'APPROVED'");
                if (depot.length() > 0) {
